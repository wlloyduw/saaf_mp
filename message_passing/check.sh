...
callservice() {
  ...
  do
    ...
    totalcalls=`echo $output | jq '.totalCalls'`
...
date
echo "Checking data spread nodecount=$1 runsperthread=$runsperthread threads=$threads totalruns=$totalruns"
echo ""
for (( i=1 ; i <= $threads ; i ++))
do
  arpt+=($runsperthread)
...
totalcost=`printf '%.*f\n' 4 $totalcost`
rm .uniqcont
echo "uuid,host,data,calls,totalcalls,uses,avgruntime_cont,avgsstuntime_cont,avglatency_cont"
...
for ((i=0;i < ${#containers[@]};i++)) {
  ...
  #echo "${containers[$i]},${chosts[$i]},${ccputype[$i]},${ccalls[$i]},${ctotalcalls[$i]},${cuses[$i]},${ctimes[$i]},$avg,$ssavg,$latencyavg,$stdiffsq"
}
